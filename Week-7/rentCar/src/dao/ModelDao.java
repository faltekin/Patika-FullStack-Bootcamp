...
public class ModelDao {
    ...
    public boolean save(Model model){
        ...
        try {
            PreparedStatement pr = this.con.prepareStatement(query);
            pr.setInt(1, model.getBrand_id());  // 1. soru işaretini değiştir
    ...
    public boolean update(Model model){
        String query = "UPDATE public.model SET " +
                " model_brand_id = ?," + // Set ile arasında boşluk olması gerekiyor
                "model_name = ?," +
                "model_type = ?," +
                "model_year = ?," +
                "model_fuel = ?," +
    ...
    public boolean delete(int model_id){
        ...
        try {
            ...
            return pr.executeUpdate() != -1; // -1 ise true döndürecek
        } catch (SQLException e){
            e.printStackTrace();
        }
        return true;
