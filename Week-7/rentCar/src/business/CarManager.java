...
public class CarManager {
    ...
    // Rezerve işlemi için arama yapma
    // ********
    public ArrayList<Car> searchForBooking(String strt_date, String fnsh_date, Model.Type type,Model.Gear gear,Model.Fuel fuel){

        /*String selectQuery = "SELECT * FROM public.car as c LEFT JOIN public.model as m ON " +
        "c.car_model_id = m.model_id WHERE m.model_fuel = ?";  */
        String query = "SELECT * FROM public.car as c LEFT JOIN public.model as m "; // LEFT JOIN car tablosu geldiğinde yanında modeliyle birlikte gelsin
        ...
        if (fuel != null){
            where.add("m.model_fuel = '"+fuel.toString()+"'"); // string olduğu için '' kullanılır
        }
        if (gear != null){
            where.add("m.model_gear = '"+gear.toString()+"'");
        }
        if (type != null){
            where.add("m.model_type = '"+type.toString()+"'"); // string olduğu için '' kullanılır
        }

        String whereStr = String.join(" AND ",where ); // Aralarına AND koyacak şekilde birleştirir
        String joinStr = String.join(" AND ",joinWhere);

        if (joinStr.length() > 0){ // join işlemi için bir koşul var ise
            query += " ON " + joinStr;
        }
        ...
        // searchedCarList arraylisti içerisinde busyCarId yani meşgul olan araba id sine sahip olanları listeden çıkartır
