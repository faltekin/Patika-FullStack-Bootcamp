...
public class ModelManager {
    ...
    public ArrayList<Object[]> getForTable(int size, ArrayList<Model> modelList){
        ...
        for (Model obj: modelList){          // Column sayısı kadar içeride bir obje oluşturduk
            int i = 0;
    ...
    public boolean delete(int id){
        if (this.getById(id) == null){ // veri tabanında böyle bir değer yoksa hata döner
    ...
    public ArrayList<Model> searchForTable(int brandId,Model.Fuel fuel,Model.Gear gear,Model.Type type){   // Arama butonuna basınca filtreleme yapmamızı sağlayacak
        String select = "SELECT * FROM public.model";
        ArrayList<String> whereList = new ArrayList<>();  // Filtreleme yaparken istersek tek bir değer üzerinden yapabilmemiz için
        if (brandId != 0){
            whereList.add("model_brand_id = "+brandId);
        }
        if (fuel != null){
            whereList.add("model_fuel ='"+fuel.toString()+"'"); // String ifade olduğu için (') işaretini koymamız gerekiyor
        }
        if (gear != null){
