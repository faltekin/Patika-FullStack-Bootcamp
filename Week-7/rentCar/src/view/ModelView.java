...
public class ModelView extends Layout{
    ...
    public ModelView(Model model){
        ...
        for (Brand brand : this.brandManager.findAll()){
            this.cmb_brand.addItem(new ComboItem(brand.getId(),brand.getName())); // Combobox içini doldurduk
        }
        ...
        if (this.model.getId() != 0){       // Alanları doldurmak için
        ...
        this.btn_model_save.addActionListener(e -> {
            if (Helper.isFieldListEmpty(new JTextField[]{this.fld_model_name,this.fld_model_year})){ // Gerekli alanların dolu olup olmadığını kontrol eder
                Helper.showMsg("fill");
            } else {
                boolean result;
                ComboItem selectedBrand = (ComboItem) cmb_brand.getSelectedItem(); // Tip dönüşümü yapmak gerekiyor
                ...
                if (this.model.getId() != 0){ // Değer varsa update işlemi yapılır - Değer yoksa save işlemi yapılır
